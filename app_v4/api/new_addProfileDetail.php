...
$user->password = "";
$user->reg_date = date('Y-m-d');
$customerDataByEmail = $user->getCustomerByEmail();
if($customerData){
    if($customerDataByEmail ){
        echo json_encode( array("status" => "Failed", "message" => "Email is already used, try with new one"));
    }else{
        if($user->updateUserInfo($data->name)){
            $user->getCustomerDataByPhone();
            $user_data = array(
                "id"    => $user->id,
                "name" => $user->name,
                "email" => $user->email,
     ...
     http_response_code(404);
            echo json_encode(
                array("status" => "Failed", "message" => "No record found")
            );
}
